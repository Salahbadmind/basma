...
async function startServer() {
  ...
  // CORS Headers to allow loading in iframes and preview environments without opaque Script errors
  ...
  // Ensure local uploads directory exists for zero-loss fallback
  const uploadsDir = path.join(process.cwd(), "public", "uploads");
  if (!fs.existsSync(uploadsDir)) {
    fs.mkdirSync(uploadsDir, { recursive: true });
  }

  // JSON Body parser for base64 / image uploads up to 50MB
  ...
  app.get("/api/health", (req, res) => {
    res.json({ status: "ok", service: "El Bahdja Dental API" });
  });
  ...
  // Cloudflare Upload Proxy (Supports Cloudflare R2 & Cloudflare Images)
  app.post("/api/cloudflare/upload", async (req, res) => {
    try {
      ...
      let cleanFileName = fileName || `file_${Date.now()}`;

      // 1. Check if source is an external URL (e.g. Unsplash or existing image)
      if (imageUrl && !dataUrl) {
        try {
      ...
      // A. ATTEMPT CLOUDFLARE R2 (S3-Compatible API)
      ...
      if (accountId && (r2SecretAccessKey || apiToken)) {
        try {
          ...
          // Build public URL from custom domain
          ...
          console.warn("Cloudflare R2 S3 Upload attempt failed:", r2Err.name, r2Err.message);
          r2UploadError = `${r2Err.name || 'Error'}: ${r2Err.message || 'Unauthorized'}`;
        }
      }

      // B. ATTEMPT CLOUDFLARE IMAGES API (Images v1)
        ...
        try {
          ...
          if (cfData.success && cfData.result) {
            ...
            if (cfData.result.variants && cfData.result.variants.length > 0) {
              ...
              cdnUrl = `${domain}/${cfData.result.id}/public`;
            } else {
          ...
          console.warn("Cloudflare Images upload failed:", cfErr.message);
        }
      }

      // C. FALLBACK: Save locally to /public/uploads/ & Return real working URL (Zero Broken Pictures)
      const localFileName = `${sanitizedName}_${Date.now()}.${ext}`;
      const localFilePath = path.join(uploadsDir, localFileName);
      fs.writeFileSync(localFilePath, buffer);

      const workingUrl = `/uploads/${localFileName}`;

      return res.json({
        success: true,
        url: dataUrl || workingUrl,
        localUrl: workingUrl,
        storage: "local",
        r2Error: r2UploadError,
        note: r2UploadError ? `Image sauvegardée localement. Erreur R2: ${r2UploadError}` : "Image sauvegardée localement."
      });
    } catch (err: any) {
    ...
    try {
      ...
      if (r2SecretAccessKey || apiToken) {
        try {
          ...
          return res.json({
            success: true,
            storageType: "r2",
            message: `✅ Connexion réussie à Cloudflare R2 ! Le bucket '${r2BucketName || "basma"}' est prêt et opérationnel. CDN public: ${domain}`,
            testUrl: `${domain}/${probeKey}`,
          });
        } catch (r2Err: any) {
          console.warn("Cloudflare test R2 failed:", r2Err.name, r2Err.message);
          r2ErrorMessage = `${r2Err.name || 'Erreur R2'}: ${r2Err.message}`;
        }
      }
      ...
      try {
        const response = await fetch(
          `https://api.cloudflare.com/client/v4/accounts/${accountId.trim()}/images/v1/stats`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${apiToken.trim()}`,
        ...
        if (data.success) {
          return res.json({
            success: true,
            storageType: "images",
            message: `✅ Connecté à Cloudflare Images avec succès ! (Images: ${data.result?.count?.current || 0})`,
            imagesCount: data.result?.count?.current || 0
          });
        }
      } catch (imgErr) {
        // Fallback
      ...
      return res.json({
        success: false,
        message: r2ErrorMessage || `Identifiants non autorisés par Cloudflare. Assurez-vous d'avoir créé un token R2 dans Cloudflare > R2 > Manage R2 API Tokens avec permissions 'Admin Read & Write' ou 'Object Read & Write'.`,
      });
    } catch (err: any) {
      return res.status(500).json({ success: false, message: err.message || "Erreur de connexion" });
    }
  });
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Dental server running on http://localhost:${PORT}`);
  });
}
